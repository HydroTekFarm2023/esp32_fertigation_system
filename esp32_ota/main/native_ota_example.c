...
#define MQTT_BROKER_URL                "mqtt://192.168.0.104"
...
static const char *TAG = "custom_native_ota_with_mqtt";
            ...
            if (NULL != strstr(event->topic, FW_UPGRADE_SUBSCRIBE_TOPIC))
            {
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     /* Starting OTA thread */
                     xTaskCreate(&ota_example_task, "ota_example_task", 8192, client, 5, NULL);
                  }
               }
            }
            else
